...
AircraftState TestFrameworkDynamics::integrate(Guidance guidance_in) {
   ...
   state.gamma = Units::RadiansAngle(
         X.gamma).value();         // flight-path angle (rad): NOTE: for gamma, heading down is positive; heading up is negative
   state.psi = Units::RadiansAngle(
         X.psi).value();         // heading angle measured from east counter-clockwise (rad); NOTE: use mathematical angle, not aviation heading
   ...
   Units::Speed v_cas = mWeatherTruth.getAtmosphere()->TAS2CAS(Units::MetersPerSecondSpeed(state.V),
                                                               Units::MetersLength(
                                                                     state.h)); // current indicated airspeed in meters per second
...
TestFrameworkDynamics::speed_on_thrust_control_dynamics(Guidance guidance_in) {
...
//	}
   ...
   Units::Speed tas_com = mWeatherTruth.getAtmosphere()->CAS2TAS(
         Units::FeetPerSecondSpeed(guidance_in.m_im_speed_command_ias),
         h);   // GUIDANCE true airspeed
   Units::Speed v_cas = mWeatherTruth.getAtmosphere()->TAS2CAS(V, h); // current indicated airspeed in meters per second
   ...
   mWeatherTruth.getAtmosphere()->airDensity(h, rho, P_tmp);
   ...
   dX.dgamma = k_gamma * (gamma_com - gamma) -
               (weather->dVwx_dh * cos(psi) + weather->dVwy_dh * sin(psi)) * pow(sin(gamma), 2) *
...
void TestFrameworkDynamics::init(double mass_percentile,
   ...
   if (initialMach != 0.0) //may fly MACH
   {
      ...
      double tas_from_ias =
            Units::FeetPerSecondSpeed(mWeatherTruth.getAtmosphere()->CAS2TAS(
   ...
   state.V = Units::MetersPerSecondSpeed(mWeatherTruth.getAtmosphere()->CAS2TAS(
   ...
   Units::Density rho;
   Units::Pressure P_tmp;
   mWeatherTruth.getAtmosphere()->airDensity(state_h, rho, P_tmp);
   // Don't bother converting P_tmp from kg/m^2 because we don't need it.
   ...
   int mode;
   double gear;
   mBadaWithCalc.getConfig(mWeatherTruth.getAtmosphere()->TAS2CAS(Units::MetersPerSecondSpeed(state.V), state_h),
                           state_h,
...
void TestFrameworkDynamics::add_to_pilot_delay(Guidance guidance_in,
                                               double time) {
   int delayed_time;
...
Guidance TestFrameworkDynamics::get_pilot_delay_guidance(Guidance prev_guidance,
                                                         double time) {
...
TestFrameworkDynamics::InternalAircraftStateD
TestFrameworkDynamics::speed_on_pitch_control_dynamics(Guidance guidance_in) {
...
//	}
   ...
   Units::Speed v_cas = mWeatherTruth.getAtmosphere()->TAS2CAS(Units::MetersPerSecondSpeed(V), Units::MetersLength(
   ...
   Units::Speed tas_com = mWeatherTruth.getAtmosphere()->CAS2TAS(ias_com, h);
   ...
   if (levelFlight) {
      ...
      if (Units::MetersLength(h).value() - Fms->constraints[Fms->NextWp].constraint_altLow > 200.0 * FT_M &&
      ...
      double esf = AircraftCalculations::ESFconstantCAS(V, h, weather->temperature);
...
void TestFrameworkDynamics::setWeatherTruth(const WeatherTruth &weatherTruth) {
   mWeatherTruth = weatherTruth;
}
