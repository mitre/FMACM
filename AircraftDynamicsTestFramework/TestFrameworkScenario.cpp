...
TestFrameworkScenario::TestFrameworkScenario() {
   ...
   m_weather = WeatherTruth(std::shared_ptr<Wind>(),
                            std::shared_ptr<Atmosphere>(new StandardAtmosphere(Units::CelsiusTemperature(0))), true);
...
bool TestFrameworkScenario::load(DecodedStream *input) {
   ...
   PostLoad(bada_data_path, predictedWindOpt, blendWind, mDefaultSimulationTimeStep);

   return true;
}

void TestFrameworkScenario::PostLoad(const string &bada_data_path, int predicted_wind_opt, bool blend_wind,
...
void TestFrameworkScenario::PostLoadAircraft(Units::Time simulation_time_step, int predicted_wind_opt,
                                             bool blend_wind) {
...
void TestFrameworkScenario::LoadOneScenarioFromScenarioClassIntoActorLists() {
   ...
   for (auto &aircraft : m_aircraft_list) {
      ...
      start_time += static_cast<int>(
            Scenario::m_rand.TruncatedGaussianSample(m_mean_inter_delivery_time, m_stdev_inter_delivery_time, 3.));
...
void TestFrameworkScenario::ProcessOneIteration() {
   ...
   fprintf(m_acstates, "Time[sec],V(ias)[m/s],V(tas)[m/s],vRate[m/s],x[m],y[m],h[m],gs[mps]\n");
...
bool TestFrameworkScenario::ProcessOneCycle(SimulationTime &time) { return ProcessAllAircraft(time); }
...
void TestFrameworkScenario::RecordState(const aaesim::open_source::AircraftState &aircraft_state,
                                        const aaesim::open_source::DynamicsState &dynamics_state) const {

   fprintf(m_acstates, "%.0f,%.3f,%.3f,%.3f,%.5f,%.5f,%.5f,%.5f\n", aircraft_state.m_time,
           Units::MetersPerSecondSpeed(dynamics_state.v_indicated_airspeed).value(),
           dynamics_state.v_true_airspeed.value(), Units::MetersPerSecondSpeed(aircraft_state.GetSpeedZd()).value(),
