...
AircraftIntentFromFile::AircraftIntentFromFile(const AircraftIntentFromFile &in)
{
	copy(in);
}

// assignment operator overload
AircraftIntentFromFile& AircraftIntentFromFile::operator=(const AircraftIntentFromFile &in)
{
	copy(in);
...
void AircraftIntentFromFile::populateWaypointsFromCsv(std::string csvfile) {
	...
	this->id = 0; // hardcoding in this test framework, must match the id in TestFrameworkAircraft.cpp
	this->number_of_waypoints = numberofrowsofdata;
	Fms.number_of_waypoints = numberofrowsofdata;
	for (int reverserow = numberofrowsofdata-1; reverserow >= 0; --reverserow) {
		Fms.LatWp[forwardrow] = Units::RadiansAngle(latwpt[reverserow]).value();
		Fms.LonWp[forwardrow] = Units::RadiansAngle(lonwpt[reverserow]).value();
		Fms.xWp[forwardrow] = xwptlocation[reverserow].value();
		Fms.yWp[forwardrow] = ywptlocation[reverserow].value();
		forwardrow++;
	}
