...
FlightManagementSystem::FlightManagementSystem(void)
{
}
...
void FlightManagementSystem::update(AircraftState state, std::vector<PrecalcWaypoint> &precalcWaypoints,
				      std::vector<HorizontalTraj> &hTraj) {
	...
	if (this->Mode == TRACKING)
	{
		...
		double BankAngle	=  LIMIT(this->DeltaTrack, -MAX_BANK_ANGLE, MAX_BANK_ANGLE);
//		this->RAD		= ( fabs(BankAngle)/ROLL_RATE_)*state.get_groundspeed_in_FPS()/2.0;


		if (BankAngle != 0.00)
		{
			this->TurnRadius		=  state.get_groundspeed_in_FPS()*state.get_groundspeed_in_FPS()/(G/FT_M*tan(BankAngle));
	...
	//gwang
	xdot = state.get_speed_in_FPS()*sin(state.get_heading())*cos(state.gamma); // + state.wind_x; // changed from cos to sin, since the value is a heading
	ydot = state.get_speed_in_FPS()*cos(state.get_heading())*cos(state.gamma); // + state.wind_y; // changed from sin to cos, since the value is a heading
	//end gwang
	...
	double currDist, tempCrs;

	AircraftCalculations::getPathLengthFromPos(state.x*FT_M,state.y*FT_M,hTraj,currDist,tempCrs);

	this->NextWp = precalcWaypoints.size() - 1;

	for (int ix = 1;(ix<precalcWaypoints.size());ix++) {

	  if ((currDist < precalcWaypoints[ix-1].constraints.constraint_dist) &&
	      (currDist >= precalcWaypoints[ix].constraints.constraint_dist)) {
	    this->NextWp = ix;
	    break;
	  }
...
void FlightManagementSystem::init()
{
	int i;
	double dx;
	double dy;
...
void FlightManagementSystem::copy_waypoints_from_intent(AircraftIntent intent_in)
{
	number_of_waypoints =  intent_in.number_of_waypoints;

	for(int j = 0; j < intent_in.number_of_waypoints; j++)
	{
		xWp[j] = intent_in.Fms.xWp[j]/FT_M;
		yWp[j] = intent_in.Fms.yWp[j]/FT_M;
		AltWp[j] = intent_in.Fms.AltWp[j]/FT_M;
		nominal_IAS_at_waypoint[j] = intent_in.Fms.nominal_IAS_at_waypoint[j];
		MACH_at_waypoint[j] = intent_in.Fms.MACH_at_waypoint[j];
		constraints[j].constraint_altHi = intent_in.Fms.altHi[j];
		constraints[j].constraint_altLow = intent_in.Fms.altLow[j];
		constraints[j].constraint_speedHi = intent_in.Fms.speedHi[j];
		constraints[j].constraint_speedLow = intent_in.Fms.speedLow[j];
	}
}

// check to see if the FMS has reached the last waypoint
bool FlightManagementSystem::is_finished()
{
	bool result = false;
...
double FlightManagementSystem::get_psi(int index)
{
