...
const Units::Time ThreeDOFDynamics::pilot_delay_mean = Units::SecondsTime(0.0);
const Units::Time ThreeDOFDynamics::pilot_delay_std = Units::SecondsTime(0.0);

ThreeDOFDynamics::ThreeDOFDynamics(void)
{
	Fms = NULL;
	model_loaded = false;
	ac_type_name = "";
...
AircraftState ThreeDOFDynamics::update(AircraftState state_in, Guidance guidance_in, double &seed)
{
	AircraftState result_state;
	Guidance pilot_delay;

	double dt0 = SimulationTime::get_simulation_time_step(); // gets simulation time step as difference in time
	Units::Time time = Units::SecondsTime(state_in.time + dt0);
	setWeatherFromTime(time);

	// add current command to the pilot delay buffer
	add_to_pilot_delay(guidance_in, state_in.time, seed);
...
AircraftState ThreeDOFDynamics::integrate(Guidance guidance_in)
{
	AircraftState result_state;
	...
	double dt0 = SimulationTime::get_simulation_time_step(); // gets simulation time step as difference in time
	Units::Time dt = Units::SecondsTime(dt0);
	...
	X.T += dX.dT * dt;
	X.phi += dX.dphi * dt;
	X.speedBrake += dX.dspeedBrake * dt0;
	...
	Units::Speed v_cas = AircraftCalculations::TAS2CAS(Units::MetersPerSecondSpeed(state.V), Units::MetersLength(state.h)); // current indicated airspeed in meters per second

	double cd0,cd2;
	int mode;
	double gear;
	ac.getConfig(Units::KnotsSpeed(v_cas).value(),state.h,Units::MetersLength(altAtFAF).value(),state.flapConfig,cd0,cd2,gear,mode);
	Units::Force maxThrust = Units::NewtonsForce(ac.getMaxThrust(state.h, mode, "cruise"));
	Units::Force minThrust = Units::NewtonsForce(ac.getMaxThrust(state.h, mode, "descent"));
...
ThreeDOFDynamics::InternalAircraftStateD ThreeDOFDynamics::speed_on_thrust_control_dynamics(Guidance guidance_in)
{
...
//	}
	...
	Units::Speed tas_com = AircraftCalculations::CAS2TAS(
                              Units::FeetPerSecondSpeed(guidance_in.indicated_airspeed),
			      h);   // GUIDANCE true airspeed
	Units::Speed v_cas = AircraftCalculations::TAS2CAS(V, h); // current indicated airspeed in meters per second
	...
	double rho1, P_tmp;
	AircraftCalculations::airDensity(Units::MetersLength(h).value(), rho1, P_tmp);
	Units::Density rho = Units::KilogramsMeterDensity(rho1);
	...
	int flapConfig_new;
	double gear;
	ac.getConfig(Units::KnotsSpeed(v_cas).value(),Units::MetersLength(h).value(),Units::MetersLength(altAtFAF).value(),flapConfig,cd0,cd2,gear,flapConfig_new);
	...
	Units::Force maxThrust = Units::NewtonsForce(
			ac.getMaxThrust(Units::MetersLength(h).value(), flapConfig_new, "cruise"));
	Units::Force minThrust = Units::NewtonsForce(
			ac.getMaxThrust(Units::MetersLength(h).value(), flapConfig_new, "descent"));
	speedBrakeCom = 0.0;
	...
	if(T_com < minThrust) {
		...
		int mode_new = 0;	
		ac.getConfigForDrag(Units::KnotsSpeed(v_cas).value(),Units::MetersLength(h).value(),Units::MetersLength(altAtFAF).value(),flapConfig_new,mode_new);
...
void ThreeDOFDynamics::setFms(FlightManagementSystem *Fms_in)
{
	Fms = Fms_in;
}

// method to check if the model loaded properly
bool ThreeDOFDynamics::is_loaded()
{
	return model_loaded;
}

void ThreeDOFDynamics::init(double mass_percentile, Units::MetersLength altAtFAF_in, Units::MetersLength initialAltitude,
			    Units::MetersPerSecondSpeed initialIas, double initialMach, double start_time)
{
	...
	if(initialMach != 0.0) //may fly MACH
	{
		...
		double tas_from_ias = 
		  Units::FeetPerSecondSpeed(AircraftCalculations::CAS2TAS(
	...
	state.V	= Units::MetersPerSecondSpeed(AircraftCalculations::CAS2TAS(
	...
	// Get temp, density, and pressure
	double rho1, P_tmp;
	AircraftCalculations::airDensity(state.h, rho1, P_tmp);
	Units::Density rho = Units::KilogramsMeterDensity(rho1);
	// Don't bother converting P_tmp from kg/m^2 because we don't need it.
	...
	int mode;
	double gear;
	ac.getConfig(Units::KnotsSpeed(AircraftCalculations::TAS2CAS(Units::MetersPerSecondSpeed(state.V), Units::MetersLength(state.h))).value(),
		     state.h,  Units::MetersLength(altAtFAF).value(), 0,cd0,cd2,gear,mode);
	...
	Units::Force maxThrust = Units::NewtonsForce(ac.getMaxThrust(state.h));
	Units::Force minThrust = Units::NewtonsForce(ac.getMaxThrust(state.h, mode, "descent"));
...
void ThreeDOFDynamics::load_env_file(string env_csv_file) {
	weatherByTime.clear();
...
void ThreeDOFDynamics::setWeatherFromTime(Units::Time time) {
...
ThreeDOFDynamics::Weather ThreeDOFDynamics::getWeatherFromTime(double time) {
...
void ThreeDOFDynamics::add_to_pilot_delay(Guidance guidance_in, double time, double &seed)
{
  int delayed_time;

#ifdef _LINUX_
  delayed_time = (int) (round(time + Units::SecondsTime(Rayleigh( ThreeDOFDynamics::pilot_delay_mean, ThreeDOFDynamics::pilot_delay_std, seed)).value())+0.1);
#else
  delayed_time = roundToInt(time + Units::SecondsTime(Rayleigh( ThreeDOFDynamics::pilot_delay_mean, ThreeDOFDynamics::pilot_delay_std, seed)).value());
#endif
...
Guidance ThreeDOFDynamics::get_pilot_delay_guidance(Guidance prev_guidance, double time)
{
...
ThreeDOFDynamics::InternalAircraftStateD ThreeDOFDynamics::speed_on_pitch_control_dynamics(Guidance guidance_in)
{
...
//	}
	...
	Units::Speed v_cas = AircraftCalculations::TAS2CAS(Units::MetersPerSecondSpeed(V), Units::MetersLength(h)); // current indicated airspeed in meters per second
	...
	Units::Speed tas_com = AircraftCalculations::CAS2TAS(ias_com, h);
	...
	{
		double esf = AircraftCalculations::ESFconstantCAS(
				Units::MetersPerSecondSpeed(V).value(), Units::MetersLength(h).value());
