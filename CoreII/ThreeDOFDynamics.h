...
class ThreeDOFDynamics : public Loadable
{
public:
	...
	void setFms(FlightManagementSystem *Fms_in); 

	// Aircraft update method that calculates the new aircraft state from the given command state
	AircraftState update(AircraftState state_in, Guidance guidance_in, double &seed);
	...
	void init(double mass_percentile,Units::MetersLength altAtFAF_in, Units::MetersLength initialAltitude, 
 ...
 private:
	...
	*/
	...
	void add_to_pilot_delay(Guidance guidance_in, double time, double &seed);
	...
	void load_env_file(string env_csv_file);

	FlightManagementSystem *Fms;

	string ac_type_name; // Aircraft type.
	...
	map<int, Guidance> pilot_delay_buffer; // Pilot Delay Buffer keyed on command time
