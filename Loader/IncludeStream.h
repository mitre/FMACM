...
public:
	...
	inline bool open_file(const string &file_name) // this over load is needed to track relative directory referance 
	{
	...
	void set_Archive_Director(RunFileArchiveDirector* new_ad)
	{
		assert(archive_director == NULL);
		archive_director = new_ad;
	}

	void set_Local_Path(FilePath fp)
	{
		local_path = fp;
	}
//=====================================================================================
...
private:
	...
	Token get_next_from_primary()
	{
		...
		if(out.get_Data() == "#include")
		{
			...
			string new_file_name = out1.get_Data();
			FilePath included_file = FilePath(new_file_name);

			bool abs_path_flag = false;
			bool cd_to_path = false;
...
bool open_secondary(const string &file_name)
	{
		...
		*secondary = *this; // this should copy all the settings 
		...
		secondary->secondary = NULL; //sets the original secondary to NULL so it doesn't point to itself and cause an infinite loop
		...
		else
		{
			...
			if(archive_director->is_new_File(fp)) // need to make an archive copy 
			{
				string where_to_put_it = archive_director->get_Destination();
				...
				string comment = string("; This file is an archive copy of a file formerly called: \"") + file_name + string("\"\n");  
	...
	RunFileArchiveDirector *archive_director;
