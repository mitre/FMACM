...
public:
	...
	inline string get_Destination()
	{
		return destination.get_Full_Path();
	}

	string get_Place_to_Write_Archive(const FilePath &source_file)
	{
		if(destination.get_Full_Path() == "")
		{
			return "";
		}
		string dest = get_New_Link_Name(source_file);
		if(dest=="")
		{
			return "";
		}
	...
	string get_New_Link_Name(const FilePath &source_file); //returns the file name that should be written into the archive copy of the file
	
	bool is_new_File(const FilePath &source_file) const;

	bool is_Name_in_Archive(const string &name) const
	{
