...
void AircraftIntent::dumpParms(std::string str) const {
   ...
   LOG4CPLUS_DEBUG(AircraftIntent::logger,
                   "No waypoints " << number_of_waypoints << "  FMS no waypoints " << number_of_waypoints << endl
                                   << endl);
   ...
   for (unsigned int i = 0; i < number_of_waypoints; i++) {
      ...
      LOG4CPLUS_DEBUG(AircraftIntent::logger,
                      "FMS ias mch " << fms.nominal_IAS_at_waypoint[i] << "  " << fms.MACH_at_waypoint[i] << endl);
...
void AircraftIntent::get_lat_lon_from_xyz(const Units::Length &xMeters,
