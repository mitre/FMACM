...
Units::KelvinTemperature Atmosphere::getTemp(const Units::Length h) const
{
//global T0 h_trop T_trop;
Units::KelvinTemperature T;
if (h < H_TROP)
    T = T0 - Units::KelvinPerMeter(6.5/1000) * h;
else
    T = T_TROP;
...
void Atmosphere::calcWindGrad(const Units::Length h_star_, const WindStack &wind, Units::Speed &w_dir, Units::Frequency &w_dir_grad) const
{
...
Units::Length Atmosphere::getMachIASTransition(const Units::Speed ias, const double mach) const
{
  ...
  return Units::FeetLength((Units::FeetLength(Units::MetersLength(1000.0)).value()/6.5)*T0.value()*(1-thetaTrans));
