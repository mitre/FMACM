...
InternalObserver::InternalObserver(void) {
    duration_adsb_transmit = duration_nav_sensor = duration_dynamics = duration_application = duration_adsb_receive = 0.;
    adsb_receiver_1st = adsb_receiver_2nd = adsb_receiver_3rd = adsb_receiver_total = 0.;
    scenario_iter = 0;
...
void InternalObserver::storeStateModel(AircraftState asv,
		int flapsConfig, float speed_brake, double ias) {
...
string InternalObserver::stateModelString(AircraftState asv, int flapsConfig, float speed_brake, double ias) {
...
void InternalObserver::dynamics_output(AircraftState dynamics_in,
                                       Guidance guidance_in) // add an entry to the dynamics list
{
	DynamicsObserver new_dynamics;
	new_dynamics.iter = scenario_iter;
	new_dynamics.id = dynamics_in.id;
...
{
    // open report data file
    string output_file_name = scenario_name + "-achieved-dynamics-output.csv";
    ofstream out;
    out.open(output_file_name.c_str());

    stable_sort(achieved_dynamics_list.begin(), achieved_dynamics_list.end()); // sort the list by aircraft and time

    //if file opens successfully, process the output
    if (out.is_open() == true && achieved_dynamics_list.empty() == false) {
        out << "Iteration,Aircraft_ID,Time,Achieved_Groundspeed(FPS),Speed_Command(FPS),IAS_Command(MPS)" << endl;

        for (unsigned int loop = 0; loop < achieved_dynamics_list.size(); loop++) {
            out << achieved_dynamics_list[loop].iter << ",";
            out << achieved_dynamics_list[loop].id << ",";
            out << achieved_dynamics_list[loop].time << ",";
            out << achieved_dynamics_list[loop].achieved_groundspeed << ",";
            out << achieved_dynamics_list[loop].speed_command << ",";
            out << achieved_dynamics_list[loop].IAS_command << endl;
        }
...
void InternalObserver::cross_output(double x_in, double y_in, double dynamic_cross, double commanded_cross,
                                    double psi_command, double phi, double limited_phi) {
    cross_entry.x = x_in;
    cross_entry.y = y_in;
...
void InternalObserver::initializeIteration(int number_of_aircraft) {
    // Performs necessary initialization for metrics needed for each
    // iteration.
    //
    // number_of_aircraft:number of aircraft in scenario.

    if (maintainStats.size() != 0)
        maintainStats.clear();

    if (finalGS.size() != 0)
        finalGS.clear();

    if (mergePointStats.size() != 0)
        mergePointStats.clear();

    if (closestPointStats.size() != 0)
        closestPointStats.clear();

    for (int ix = 0; ix < number_of_aircraft; ix++) {
...
void InternalObserver::addPredictedWind(int id, const WindStack &predWindX, const WindStack &predWindY) {
...
string InternalObserver::predWindsData(int id, int col, string field, const WindStack &mat) {
...
void InternalObserver::addAchieveRcd(size_t aircraftId,double tm,
...
void InternalObserver::dumpOwnKinTraj(int id, VerticalPath &fullTraj) {
...
void InternalObserver::dumpTargetKinTraj(int id, VerticalPath &fullTraj) {
