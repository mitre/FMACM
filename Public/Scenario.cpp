...
const Units::NauticalMilesLength Scenario::DEFAULT_ADS_B_RECEPTION_RANGE_THRESHOLD = Units::NauticalMilesLength(90.0);

Scenario::Scenario() : m_scenario_name() {}

Scenario::~Scenario() = default;

bool Scenario::load(DecodedStream *input) { return true; }

void Scenario::SetScenarioName(const string &in) {
   m_scenario_name.assign(in);

   // remove the leading directory structure if present (search for last instance of "/" or "\\")
   unsigned long index = m_scenario_name.find_last_of("/\\");
...
void Scenario::DuplicateAcidCheck(const size_t aircraft_count) {
   ...
   if (aircraft_count != aircraft_id_count) {
      string msg = "Scenario has " + std::to_string(aircraft_count) + " aircraft but " +
