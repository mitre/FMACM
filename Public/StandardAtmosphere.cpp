...
StandardAtmosphere *StandardAtmosphere::MakeInstance(const Units::KelvinTemperature temperature,
                                                     const Units::Length altitude) {
   if (temperature < T_TROP) {
      LOG4CPLUS_DEBUG(m_logger, "Creating non-standard StandardAtmosphere with temperature "
                                      << temperature << " at " << Units::FeetLength(altitude)
                                      << ".  Tropopause temperature is " << T_TROP);
   }
   ...
   if (temperature == T_TROP) {
      LOG4CPLUS_WARN(m_logger, "StandardAtmosphere created using tropopause temperature "
                                     << T_TROP << " at " << Units::FeetLength(altitude)
                                     << ".  Assuming that represents the floor of the tropopause.");
   }
...
StandardAtmosphere *StandardAtmosphere::MakeInstanceFromTemperatureOffset(
      Units::CelsiusTemperature temperature_offset) {
...
void StandardAtmosphere::SetTemperatureOffset(const Units::Temperature temperatureOffset) {
   ...
   m_tropopause_height =
         Units::MetersLength(11000) + Units::MetersLength(1000) * m_temperature_offset / Units::CelsiusTemperature(6.5);
...
Units::Temperature StandardAtmosphere::GetTemperatureOffset() const { return m_temperature_offset; }
...
Units::KelvinTemperature StandardAtmosphere::GetSeaLevelTemperature() const { return m_sea_level_temperature; }

Units::MetersLength StandardAtmosphere::GetTropopauseHeight() const { return m_tropopause_height; }

Units::Density StandardAtmosphere::GetSeaLevelDensity() const { return m_sea_level_density; }

Units::Density StandardAtmosphere::GetTropopauseDensity() const { return m_tropopause_density; }

Units::Pressure StandardAtmosphere::GetTropopausePressure() const { return m_tropopause_pressure; }
