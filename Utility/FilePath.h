...
public:
	...
	inline bool operator==(const FilePath& rhs) const
	{
		...
		if(num_wild > 0)
		{
			...
			{
				if(rhs.full_path.find(full_path.substr(0,found-1))!=string::npos)
				{
					return true;
				}
			}
		...
		if(rhs.num_wild > 0)
		{
			...
			if(found==0)
			{
				string end = rhs.full_path.substr(1);
	...
	string get_Full_Path() const ; //returns the full path as a string
	string get_Type() const;	// returns the part after the . if there is not extension then return an empty string
						// for a file name like this foo.bar.txt this returns txt
	string get_Disk() const;	// this returns the disk if it is specified
						// FilePath(“C:king/crab.h”).get_Disk() -> C
	string remove_Last_Dir() const; // returns the popped value
	int get_Num_Dir() const;
	vector<string> list_Dirs() const;
...
private:
	...
	int num_wild;
	bool error;
	string file_name; //for C:/foo/bar.txt, the file name is bar.txt

	
	//very private, only to be used in the constructor///////////
	int extract_disk(const string& name);                      // 
	int extract_type(const string& name);                      // 
	void extract_path(const string& name, int& index, int end);//
	void extract_pop(const string& name);                      //
	int get_Number_Of_Wild_Cards(const string& name);          //
	/////////////////////////////////////////////////////////////
