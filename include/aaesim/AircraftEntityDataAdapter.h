...
namespace aaesim {
class AircraftEntityDataAdapter final : public aaesim::DataGatherer {
  public:
   AircraftEntityDataAdapter(std::shared_ptr<aaesim::AircraftDataIterationWriter> data_writer)
      : m_data_writer(data_writer) {}
   ~AircraftEntityDataAdapter() = default;
   void GatherData(const int iteration_number, std::shared_ptr<const aaesim::AircraftEntity> aircraft_entity) override {
      m_data_writer->Gather(iteration_number, *aircraft_entity);
   }
  ...
  private:
   std::shared_ptr<aaesim::AircraftDataIterationWriter> m_data_writer;
};
}  // namespace aaesim