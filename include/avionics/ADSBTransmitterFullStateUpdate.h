...
class ADSBTransmitterFullStateUpdate final : public aaesim::open_source::ADSBTransmitter {
  public:
   ADSBTransmitterFullStateUpdate(int nacp, int nacv, int nicp, int nicv, bool latency_is_known, int latency_factor,
                                  bool use_latency_compensation, Units::SecondsTime adsb_update_period,
                                  Units::SecondsTime adsb_std_dev_time_of_applicability);
   ~ADSBTransmitterFullStateUpdate() = default;
  ...
  private:
   inline static log4cplus::Logger m_logger{
         log4cplus::Logger::getInstance(LOG4CPLUS_TEXT("ADSBTransmitterFullStateUpdate"))};
...
inline const std::vector<aaesim::open_source::ADSBSVReport> &ADSBTransmitterFullStateUpdate::GetAllTransmissions()
      const {
