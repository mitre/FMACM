...
class ADSBTransmitterTimeLatency final : public aaesim::open_source::ADSBTransmitter {
  public:
   ADSBTransmitterTimeLatency(int nacp, int nacv, int nicp, int nicv, bool latency_is_known, int latency_factor,
                              bool use_latency_compensation, Units::SecondsTime adsb_update_period,
                              Units::SecondsTime adsb_std_dev_time_of_applicability);
   ~ADSBTransmitterTimeLatency() = default;
  ...
  private:
   inline static log4cplus::Logger m_logger{
         log4cplus::Logger::getInstance(LOG4CPLUS_TEXT("ADSBTransmitterTimeLatency"))};
...
inline const std::vector<aaesim::open_source::ADSBSVReport> &ADSBTransmitterTimeLatency::GetAllTransmissions() const {
