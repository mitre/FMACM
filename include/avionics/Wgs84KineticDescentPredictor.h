...
namespace aaesim {
class Wgs84KineticDescentPredictor : public Wgs84TrajectoryPredictor {
  public:
   Wgs84KineticDescentPredictor()
      : m_horizontal_path(),
        m_waypoint_vector(),
        m_bank_angle(),
        m_altitude_at_final_waypoint(),
        m_aircraft_distance_to_go(),
        m_aircraft_intent(),
        m_distance_calculator(),
        m_position_calculator(),
        m_atmosphere(),
        m_vertical_predictor(),
        m_aircraft_type(){};

   Wgs84KineticDescentPredictor(Units::Angle max_bank_below_fl195,
                                std::shared_ptr<Wgs84KineticDescent4DPredictor> vertical_predictor);

   Wgs84KineticDescentPredictor(const Wgs84KineticDescentPredictor &obj);

   virtual ~Wgs84KineticDescentPredictor() = default;

   bool operator==(const Wgs84KineticDescentPredictor &obj) const;

   bool operator!=(const Wgs84KineticDescentPredictor &obj) const;

   Wgs84KineticDescentPredictor &operator=(const Wgs84KineticDescentPredictor &obj) = default;
   ...
   const std::vector<Wgs84HorizontalPathSegment> EstimateHorizontalTrajectory(
  ...
  protected:
   ...
   void Copy(const Wgs84KineticDescentPredictor &obj);

   std::vector<Wgs84HorizontalPathSegment> m_horizontal_path;
   std::vector<Wgs84PrecalcWaypoint> m_waypoint_vector;
   ...
   std::shared_ptr<Wgs84KineticDescent4DPredictor> m_vertical_predictor;

  private:
   // Allowable course change, in degrees, to be considered straight, and does not need turn anticipation
   // Also the allowable course change beween RF-Leg course and previous or next course to consider aligned.
...
inline std::shared_ptr<Wgs84KineticDescent4DPredictor> Wgs84KineticDescentPredictor::GetVertPredictor() const {
   return std::shared_ptr<Wgs84KineticDescent4DPredictor>(
         std::static_pointer_cast<Wgs84KineticDescent4DPredictor>(m_vertical_predictor));
}

inline const std::string &Wgs84KineticDescentPredictor::GetAircraftType() const { return m_aircraft_type; }

inline const std::shared_ptr<Atmosphere> Wgs84KineticDescentPredictor::GetAtmosphere() const { return m_atmosphere; }

inline const std::vector<Wgs84PrecalcWaypoint> &Wgs84KineticDescentPredictor::GetPrecalcWaypoints() const {
   return m_waypoint_vector;
}

inline Units::Angle Wgs84KineticDescentPredictor::GetBankAngle() const { return m_bank_angle; }

inline Units::Length Wgs84KineticDescentPredictor::GetAltitudeAtFinalWaypoint() const {
   return m_altitude_at_final_waypoint;
}

inline const std::vector<Wgs84HorizontalPathSegment> &Wgs84KineticDescentPredictor::GetHorizontalPath() const {
   return m_horizontal_path;
}

inline const AircraftIntent &Wgs84KineticDescentPredictor::GetAircraftIntent() const { return m_aircraft_intent; }

inline const std::vector<Wgs84HorizontalPathSegment> Wgs84KineticDescentPredictor::EstimateHorizontalTrajectory(
      const aaesim::open_source::WeatherPrediction &weather_prediction) {
...
inline void Wgs84KineticDescentPredictor::SetAtmosphere(std::shared_ptr<Atmosphere> atmosphere) {
   m_atmosphere = atmosphere;
   m_vertical_predictor->SetAtmosphere(atmosphere);
}

inline const VerticalPath Wgs84KineticDescentPredictor::GetVerticalPath() const {
