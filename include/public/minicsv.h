...
// This was downloaded from https://github.com/shaovoon/minicsv
...
namespace mini {
   ...
   namespace csv {
      inline std::string const &replace(std::string &src,
      ...
      public:
         sep(const char delimiter_,
             const std::string &escape_)
               : delimiter(delimiter_), escape(escape_) {
         }
      ...
      public:
         ifstream()
               : str(""), pos(0), delimiter(","), unescape_str("##"), trim_quote_on_str(false), trim_quote('\"'),
                 terminate_on_blank_line(true), quote_unescape("&quot;"), has_bom(false), first_line_read(false),
                 filename(""), line_num(0), token_num(0) {
         }
         ...
         void read_bom() {
            ...
            if (tt[0] == (char) 0xEF || tt[1] == (char) 0xBB ||
                tt[2] == (char) 0xBF) { // not the correct BOM, so reset the pos to beginning (file might not have BOM)
               has_bom = true;
            }
         ...
         void enable_trim_quote_on_str(bool enable,
                                       char quote,
         ...
         void set_delimiter(char delimiter_,
                            std::string const &unescape_str_) {
         ...
         std::string get_delimited_str() {
            ...
            do {
               ...
               if (trim_quote_on_str) {
                  if (within_quote == false && ch == trim_quote &&
                      ((pos > 0 && this->str[pos - 1] == delimiter[0]) || pos == 0)) {
                     within_quote = true;
         ...
         std::string error_line(const std::string &token,
                                const std::string &function_site) {
      ...
      public:
         ...
         ofstream()
               : after_newline(true), delimiter(","), escape_str("##"), surround_quote_on_str(false),
                 surround_quote('\"'), quote_escape("&quot;") {
            std::ostringstream tmp;
            mPrecision = tmp.precision();
         }
         ...
         void enable_surround_quote_on_str(bool enable,
                                           char quote,
                                           const std::string &escape = "&quot;") {
...
template<>
inline mini::csv::ifstream &operator>>(mini::csv::ifstream &istm,
                                       std::string &val) {
...
mini::csv::ofstream &operator<<(mini::csv::ofstream &ostm,
   ...
   os_temp.precision(ostm.precision());
...
template<>
inline mini::csv::ofstream &operator<<(mini::csv::ofstream &ostm,
   ...
   namespace csv {
      ...
      public:
         istringstream(const char *text)
               : str(""), pos(0), delimiter(","), unescape_str("##"), trim_quote_on_str(false), trim_quote('\"'),
                 terminate_on_blank_line(true), quote_unescape("&quot;"), line_num(0), token_num(0) {
            istm.str(text);
         }
...
template<>
inline mini::csv::istringstream &operator>>(mini::csv::istringstream &istm,
                                            std::string &val) {
   val = istm.get_delimited_str();
