...
TEST(Utility_Functions, sgn) {
   EXPECT_DOUBLE_EQ(sgn<double>(1), 1);      // positive
   EXPECT_DOUBLE_EQ(sgn<double>(-1), -1);    // negative
   EXPECT_DOUBLE_EQ(sgn<double>(0.5), 1);    // positive
   EXPECT_DOUBLE_EQ(sgn<double>(-0.5), -1);  // negative
   EXPECT_DOUBLE_EQ(sgn<double>(0), 0);      // zero
}
