...
namespace aaesim
{
   namespace test
   {
      namespace open_source
      {
         ...
         {
            LatitudeLongitudePoint start_point(Units::SignedDegreesAngle(4),
                                               Units::SignedRadiansAngle(-10));
            LatitudeLongitudePoint end_point(Units::SignedDegreesAngle(3),
                                             Units::SignedRadiansAngle(-103));

            const LineOnEllipsoid geodesic = LineOnEllipsoid::CreateFromPoints(start_point, end_point);

            // This is the tested method ----
            const LatitudeLongitudePoint calculated_point = geodesic.CalculatePointAtDistanceFromStartPoint(geodesic.GetShapeLength());
            EXPECT_NEAR(Units::RadiansAngle(end_point.GetLatitude()).value(), Units::RadiansAngle(calculated_point.GetLatitude()).value(), 1e-8);
            EXPECT_NEAR(Units::RadiansAngle(end_point.GetLongitude()).value(), Units::RadiansAngle(calculated_point.GetLongitude()).value(), 1e-8);
            // ------------------------------
         }
         ...
         TEST(LineOnEllipsoid, calculate_course_at_point_on_shape)
         {
            ...
            // This is the tested method ----
            const std::pair<Units::SignedAngle, LatitudeLongitudePoint> calculated_course_and_point = geodesic.CalculateCourseAtDistanceFromStartPoint(geodesic.GetShapeLength());
            ...
            EXPECT_NEAR(Units::SignedRadiansAngle(geodesic.GetForwardCourseEnuAtEndPoint()).value(), actual_course.value(), 1e-8);
            EXPECT_NEAR(Units::RadiansAngle(end_point.GetLatitude()).value(), Units::RadiansAngle(actual_point.GetLatitude()).value(), 1e-8);
            EXPECT_NEAR(Units::RadiansAngle(end_point.GetLongitude()).value(), Units::RadiansAngle(actual_point.GetLongitude()).value(), 1e-8);
            // ------------------------------
         }
         ...
         TEST(LineOnEllipsoid, createLineFromTwoPoints)
         {
            ...
            ErrorSet error_set_inverse = inverse(
                ...
                &reverse_course_radians_at_dest_calculated,
                &distance_nm_calculated,
                DEFAULT_EPS);
            ...
            EXPECT_NEAR(course_ned_radians_at_origin_calculated, Units::UnsignedRadiansAngle(GeolibUtils::ConvertCourseFromEnuToNed(geodesic.GetForwardCourseEnuAtStartPoint())).value(), 1e-12);
            const double expected_forward_course_ned = modpos(reverse_course_radians_at_dest_calculated + M_PI, M_2PI); // geolib inverse gives the reverse the course, so we need to do adjust to forward here
            EXPECT_NEAR(expected_forward_course_ned, Units::UnsignedRadiansAngle(GeolibUtils::ConvertCourseFromEnuToNed(geodesic.GetForwardCourseEnuAtEndPoint())).value(), 1e-12);
         }
         ...
         TEST(GeolibUtils, createArcFromThreeKnownPoints)
         {
            ...
            const LatitudeLongitudePoint center_point(Units::SignedDegreesAngle(33.3862),
                                                      Units::SignedRadiansAngle(-111.887));
            LLPoint start_point_calculated;
            ErrorSet error_set_1 = direct(
                center_point.GetGeolibPrimitiveLLPoint(),
                0,
                expected_radius.value(),
                &start_point_calculated,
                DEFAULT_EPS);
            ...
            ErrorSet error_set_2 = direct(
                center_point.GetGeolibPrimitiveLLPoint(),
                M_PI / 2,
                expected_radius.value(),
                &end_point_calculated,
                DEFAULT_EPS);
         ...
         TEST(LatitudeLongitudePoint, calculate_new_point_1nm)
         {
            ...
            ErrorSet error_set_inverse = inverse(
                start_point.GetGeolibPrimitiveLLPoint(),
                calculated_point.GetGeolibPrimitiveLLPoint(),
                &crs_radians_ned_truth,
                &tmp_course_radians_calculated,
                &distance_nm_truth,
                GEOLIB_EPSILON);
            if (error_set_inverse != SUCCESS)
            {
            ...
            EXPECT_NEAR(crs_radians_ned_truth, Units::UnsignedRadiansAngle(GeolibUtils::ConvertCourseFromEnuToNed(go_north_enu)).value(), 1e-12);
         }
         ...
         TEST(LatitudeLongitudePoint, calculate_new_point_1meter)
         {
            ...
            EXPECT_NEAR(crs_radians_ned_truth, Units::UnsignedRadiansAngle(GeolibUtils::ConvertCourseFromEnuToNed(go_east_enu)).value(), 1e-12);
         }
         ...
         TEST(GeolibUtils, test_CalculateNewPoint)
         {
            ...
            ErrorSet error_set_inverse = inverse(
                start_point.GetGeolibPrimitiveLLPoint(),
                calculated_point.GetGeolibPrimitiveLLPoint(),
                &crs_radians_truth_ned_unsigned,
                &tmp_course_radians_calculated,
                &distance_nm_truth,
                epsilon);
            if (error_set_inverse != SUCCESS)
            ...
            EXPECT_NEAR(distance_nm_truth, Units::NauticalMilesLength(distance).value(), 1e-12);
            EXPECT_NEAR(crs_radians_truth_ned_unsigned, Units::SignedRadiansAngle(GeolibUtils::ConvertCourseFromEnuToNed(go_east_enu)).value(), 1e-12);
         }
         ...
         TEST(LatitudeLongitudePoint, test_RelationshipBewteenPoints_1meter)
         {
            ...
            ErrorSet error_set_direct = direct(
                ...
                Units::NauticalMilesLength(distance).value(),
                &end_point_from_geolib,
                GEOLIB_EPSILON);
            if (error_set_direct != SUCCESS)
            {
               std::cout << "BLAH. Something went wrong with the direct operation! "
                         << formatErrorMessage(error_set_direct)
                         << std::endl;
               FAIL();
            }
            ...
            // This is the tested method ----
            std::pair<Units::MetersLength, Units::SignedRadiansAngle> point_relationship = start_point.CalculateRelationshipBetweenPoints(end_point);
            // ------------------------------
         ...
         TEST(GeolibUtils, test_RelationshipBewteenPoints_1meter)
         {
            ...
            // This is the tested method ----
            std::pair<Units::MetersLength, Units::SignedRadiansAngle> point_relationship = GeolibUtils::CalculateRelationshipBetweenPoints(start_point, end_point);
            // ------------------------------
         ...
         TEST(GeolibUtils, test_RelationshipBewteenPoints_1nm)
         {
            ...
            // This is the tested method ----
            std::pair<Units::NauticalMilesLength, Units::SignedRadiansAngle> point_relationship = GeolibUtils::CalculateRelationshipBetweenPoints(start_point, end_point);
            // ------------------------------
         ...
         TEST(GeolibUtils, test_ConvertCourseFromEnuToNed)
         {
            ...
            zipped_angles_input_then_expected.push_back(std::pair<Units::SignedDegreesAngle, Units::UnsignedDegreesAngle>(Units::SignedDegreesAngle(0), Units::UnsignedDegreesAngle(90)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::SignedDegreesAngle, Units::UnsignedDegreesAngle>(Units::SignedDegreesAngle(45), Units::UnsignedDegreesAngle(45)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::SignedDegreesAngle, Units::UnsignedDegreesAngle>(Units::SignedDegreesAngle(90), Units::UnsignedDegreesAngle(0)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::SignedDegreesAngle, Units::UnsignedDegreesAngle>(Units::SignedDegreesAngle(135), Units::UnsignedDegreesAngle(315)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::SignedDegreesAngle, Units::UnsignedDegreesAngle>(Units::SignedDegreesAngle(-45), Units::UnsignedDegreesAngle(135)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::SignedDegreesAngle, Units::UnsignedDegreesAngle>(Units::SignedDegreesAngle(-90), Units::UnsignedDegreesAngle(180)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::SignedDegreesAngle, Units::UnsignedDegreesAngle>(Units::SignedDegreesAngle(-95), Units::UnsignedDegreesAngle(185)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::SignedDegreesAngle, Units::UnsignedDegreesAngle>(Units::SignedDegreesAngle(180), Units::UnsignedDegreesAngle(270)));
         ...
         TEST(GeolibUtils, test_ConvertCourseFromNedToEnu)
         {
            ...
            zipped_angles_input_then_expected.push_back(std::pair<Units::UnsignedDegreesAngle, Units::SignedDegreesAngle>(Units::UnsignedDegreesAngle(90), Units::SignedDegreesAngle(0)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::UnsignedDegreesAngle, Units::SignedDegreesAngle>(Units::UnsignedDegreesAngle(45), Units::SignedDegreesAngle(45)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::UnsignedDegreesAngle, Units::SignedDegreesAngle>(Units::UnsignedDegreesAngle(0), Units::SignedDegreesAngle(90)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::UnsignedDegreesAngle, Units::SignedDegreesAngle>(Units::UnsignedDegreesAngle(315), Units::SignedDegreesAngle(135)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::UnsignedDegreesAngle, Units::SignedDegreesAngle>(Units::UnsignedDegreesAngle(135), Units::SignedDegreesAngle(-45)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::UnsignedDegreesAngle, Units::SignedDegreesAngle>(Units::UnsignedDegreesAngle(180), Units::SignedDegreesAngle(-90)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::UnsignedDegreesAngle, Units::SignedDegreesAngle>(Units::UnsignedDegreesAngle(185), Units::SignedDegreesAngle(-95)));
            zipped_angles_input_then_expected.push_back(std::pair<Units::UnsignedDegreesAngle, Units::SignedDegreesAngle>(Units::UnsignedDegreesAngle(270), Units::SignedDegreesAngle(180)));
         ...
         TEST(GeolibUtils, test_CalculateLineLineIntersectionPoint)
         {
            ...
            const LatitudeLongitudePoint end_point1 = GeolibUtils::CalculateNewPoint(expected_point, dist1, Units::DegreesAngle(0));
            const LineOnEllipsoid tmp_line = LineOnEllipsoid::CreateFromPoints(expected_point, end_point1);
            const LatitudeLongitudePoint new_start_point1 = GeolibUtils::CalculateNewPoint(expected_point,
                                                                                           dist1,
                                                                                           tmp_line.GetForwardCourseEnuAtStartPoint() + Units::RadiansAngle(M_PI));
            ...
            const LatitudeLongitudePoint end_point2 = GeolibUtils::CalculateNewPoint(expected_point, dist2, Units::DegreesAngle(10));
            ...
            EXPECT_NEAR(Units::SignedRadiansAngle(expected_point.GetLatitude()).value(), Units::SignedRadiansAngle(calculated_intersection_point.GetLatitude()).value(), 1e-10);
            EXPECT_NEAR(Units::SignedRadiansAngle(expected_point.GetLongitude()).value(), Units::SignedRadiansAngle(calculated_intersection_point.GetLongitude()).value(), 1e-10);
         ...
         TEST(GeolibUtils, test_NoPossibleLineLineIntersectionPoint)
         {
            ...
            const LatitudeLongitudePoint end_point1 = GeolibUtils::CalculateNewPoint(start_point1, Units::NauticalMilesLength(.1), Units::DegreesAngle(0));
            ...
            const LatitudeLongitudePoint end_point2 = GeolibUtils::CalculateNewPoint(start_point2, Units::NauticalMilesLength(.5), Units::DegreesAngle(10));
         ...
         TEST(GeolibUtils, test_IsPointOnLine)
         {
            ...
            const LatitudeLongitudePoint point_to_east = GeolibUtils::CalculateNewPoint(test_point, distance, Units::SignedDegreesAngle(10));
            const LineOnEllipsoid tmp_line = LineOnEllipsoid::CreateFromPoints(test_point, point_to_east);
            const LatitudeLongitudePoint start_point = GeolibUtils::CalculateNewPoint(test_point,
                                                                                      distance,
                                                                                      tmp_line.GetForwardCourseEnuAtStartPoint() + Units::SignedRadiansAngle(M_PI));
         ...
         TEST(GeolibUtils, test_PoinIsNotOnLine)
         {
            ...
            const Units::NauticalMilesLength distance = Units::NauticalMilesLength(1);
            const LatitudeLongitudePoint point_to_east = GeolibUtils::CalculateNewPoint(line_mid_point, distance, Units::SignedDegreesAngle(10));
            const LineOnEllipsoid tmp_line = LineOnEllipsoid::CreateFromPoints(line_mid_point, point_to_east);
            const LatitudeLongitudePoint start_point = GeolibUtils::CalculateNewPoint(line_mid_point,
                                                                                      distance,
                                                                                      tmp_line.GetForwardCourseEnuAtStartPoint() + Units::SignedRadiansAngle(M_PI));
            ...
            const LatitudeLongitudePoint test_point = GeolibUtils::CalculateNewPoint(line_mid_point, Units::MetersLength(.001), Units::SignedDegreesAngle(0));
         ...
         TEST(GeolibUtils, test_IsPointOnArc)
         {
            ...
            const LatitudeLongitudePoint center_point(Units::SignedDegreesAngle(33.3862),
                                                      Units::SignedRadiansAngle(-111.887));
            const LatitudeLongitudePoint start_point = GeolibUtils::CalculateNewPoint(center_point, defined_radius, Units::SignedDegreesAngle(90));
            const LatitudeLongitudePoint end_point = GeolibUtils::CalculateNewPoint(center_point, defined_radius, Units::SignedDegreesAngle(0));
            const ArcOnEllipsoid arc_on_ellipsoid =
                GeolibUtils::CreateArcOnEllipsoid(start_point, end_point, center_point, ArcDirection::CLOCKWISE);
            const LatitudeLongitudePoint point_on_arc = GeolibUtils::CalculateNewPoint(center_point, defined_radius, Units::SignedDegreesAngle(45));
         ...
         TEST(GeolibUtils, test_PoinIsNotOnArc)
         {
            ...
            const LatitudeLongitudePoint point_not_on_arc = GeolibUtils::CalculateNewPoint(center_point, defined_radius - Units::MetersLength(0.001), Units::SignedDegreesAngle(45));
         ...
         TEST(ArcOnEllipsoid, calculate_point_on_shape)
         {
            ...
            // This is the tested method ----
            const LatitudeLongitudePoint calculated_point = arc_on_ellipsoid.CalculatePointAtDistanceFromStartPoint(arc_on_ellipsoid.GetShapeLength());
            EXPECT_NEAR(Units::RadiansAngle(end_point.GetLatitude()).value(), Units::RadiansAngle(calculated_point.GetLatitude()).value(), 1e-8);
            EXPECT_NEAR(Units::RadiansAngle(end_point.GetLongitude()).value(), Units::RadiansAngle(calculated_point.GetLongitude()).value(), 1e-8);
            // ------------------------------
         }
         ...
         TEST(ArcOnEllipsoid, calculate_course_at_point_on_shape)
         {
            ...
            // This is the tested method ----
            const std::pair<Units::SignedRadiansAngle, LatitudeLongitudePoint> calculated_course_and_point_at_distance = arc_on_ellipsoid.CalculateCourseAtDistanceFromStartPoint(arc_on_ellipsoid.GetShapeLength());
            EXPECT_NEAR(Units::SignedRadiansAngle(arc_on_ellipsoid.GetCourseEnuTangentToEndPoint()).value(), calculated_course_and_point_at_distance.first.value(), 1e-8);
            EXPECT_NEAR(Units::RadiansAngle(end_point.GetLatitude()).value(), Units::RadiansAngle(calculated_course_and_point_at_distance.second.GetLatitude()).value(), 1e-8);
            EXPECT_NEAR(Units::RadiansAngle(end_point.GetLongitude()).value(), Units::RadiansAngle(calculated_course_and_point_at_distance.second.GetLongitude()).value(), 1e-8);
            // ------------------------------
         }
         ...
         TEST(GeolibUtils, FindPointOnLineUsingPerpendiculorProjection_valid)
         {
            ...
            const LatitudeLongitudePoint test_point = GeolibUtils::CalculateNewPoint(line_mid_point,
                                                                                     Units::MetersLength(.001),
            ...
            EXPECT_NEAR(Units::RadiansAngle(line_mid_point.GetLatitude()).value(), Units::RadiansAngle(point_on_line.GetLatitude()).value(), 1e-10);
            EXPECT_NEAR(Units::RadiansAngle(line_mid_point.GetLongitude()).value(), Units::RadiansAngle(point_on_line.GetLongitude()).value(), 1e-10);
         }
         ...
         TEST(GeolibUtils, IsPointInsideArc)
         {
            ...
            const ArcOnEllipsoid arc_on_ellipsoid =
                GeolibUtils::CreateArcOnEllipsoid(start_point, end_point, center_point, ArcDirection::CLOCKWISE);
            const LatitudeLongitudePoint point_inside_arc = GeolibUtils::CalculateNewPoint(center_point, defined_radius - Units::MetersLength(1e-5), Units::SignedDegreesAngle(45));
         ...
         TEST(GeolibUtils, IsPointInsideArc_false)
         {
            ...
            const ArcOnEllipsoid arc_on_ellipsoid =
                GeolibUtils::CreateArcOnEllipsoid(start_point, end_point, center_point, ArcDirection::CLOCKWISE);
            const LatitudeLongitudePoint point_not_inside_arc = GeolibUtils::CalculateNewPoint(center_point, defined_radius + Units::MetersLength(1e-5), Units::SignedDegreesAngle(45));
         ...
         TEST(GeolibUtils, FindNearestPointOnArc)
         {
            ...
            const ArcOnEllipsoid arc_on_ellipsoid =
                GeolibUtils::CreateArcOnEllipsoid(start_point, end_point, center_point, ArcDirection::CLOCKWISE);
            const LatitudeLongitudePoint point_on_arc_to_find = GeolibUtils::CalculateNewPoint(center_point, defined_radius, Units::SignedDegreesAngle(45));
            const LatitudeLongitudePoint point_outside_arc = GeolibUtils::CalculateNewPoint(center_point, defined_radius + Units::NauticalMilesLength(1), Units::SignedDegreesAngle(45));

            // This is the tested method ----
            std::pair<bool, const LatitudeLongitudePoint> ret = GeolibUtils::FindNearestPointOnArcUsingPerpendiculorProjection(arc_on_ellipsoid, point_outside_arc);
            // ------------------------------
            ...
            EXPECT_NEAR(Units::RadiansAngle(point_on_arc_to_find.GetLatitude()).value(), Units::RadiansAngle(ret.second.GetLatitude()).value(), 1e-10);
            EXPECT_NEAR(Units::RadiansAngle(point_on_arc_to_find.GetLongitude()).value(), Units::RadiansAngle(ret.second.GetLongitude()).value(), 1e-10);
         }
         ...
         TEST(GeolibUtils, PointsAreNotMathematicallyEqual)
         {
            ...
            const LatitudeLongitudePoint point1(Units::SignedDegreesAngle(34.23423432341236),
                                                Units::SignedDegreesAngle(-112.1234123569987));
            const LatitudeLongitudePoint point_very_close_to_point1 = point1.ProjectDistanceAlongCourse(Units::MetersLength(1e-5), Units::DegreesAngle(2));
         ...
         TEST(GeolibUtils, CreateArcTangentToTwoLines_valid_case)
         {
            ...
            std::vector<std::tuple<bool, Waypoint, Units::Length>> waypoints_to_connect = {
                ...
                std::make_tuple(true, Waypoint("HIMOM", Units::DegreesAngle(39.788108), Units::DegreesAngle(-104.808950), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::infinity()),
                std::make_tuple(true, Waypoint("MCMUL", Units::DegreesAngle(39.738073), Units::DegreesAngle(-104.810275), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::infinity()),
                std::make_tuple(false, Waypoint("KUGLN", Units::DegreesAngle(39.691536), Units::DegreesAngle(-104.752289), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::NauticalMilesLength(2.8)),
                std::make_tuple(false, Waypoint("BSAYN", Units::DegreesAngle(39.739938), Units::DegreesAngle(-104.688628), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::NauticalMilesLength(2.9)),
                std::make_tuple(true, Waypoint("CORDE", Units::DegreesAngle(39.780965), Units::DegreesAngle(-104.688147), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::infinity()),
            };
            ...
            for (auto item : waypoints_to_connect)
            {
               ...
               if (wpt.GetName() != "HIMOM")
               {
                  ...
                  if (is_line)
                  {
                     LineOnEllipsoid tf_line = LineOnEllipsoid::CreateFromPoints(LatitudeLongitudePoint::CreateFromWaypoint(prev_waypoint), LatitudeLongitudePoint::CreateFromWaypoint(wpt));
                  ...
                  else
                  {
                     ...
                     const LatitudeLongitudePoint projected_pt_after_rf_began = where_rf_began.ProjectDistanceAlongCourse(Units::NauticalMilesLength(100), course_enu_tangency_end_of_prev_shape);
                     const LineOnEllipsoid fake_line_behind_rf_leg = LineOnEllipsoid::CreateFromPoints(where_rf_began, projected_pt_after_rf_began);
                     const LatitudeLongitudePoint where_rf_ends = LatitudeLongitudePoint::CreateFromWaypoint(wpt);
                     std::tuple<LatitudeLongitudePoint, Units::SignedAngle, Units::Length> perp_info =
                         GeolibUtils::FindNearestPointOnLineUsingPerpendicularProjection(fake_line_behind_rf_leg, where_rf_ends);
                     const LatitudeLongitudePoint intersection_point_for_rf_outbound_leg = std::get<0>(perp_info);
                     const LineOnEllipsoid fake_line_outbound_from_rf_leg = LineOnEllipsoid::CreateFromPoints(intersection_point_for_rf_outbound_leg, where_rf_ends);
                     const LineOnEllipsoid fake_line_outbound_from_rf_leg_extended = fake_line_outbound_from_rf_leg.CreateExtendedLine(Units::NauticalMilesLength(100));

                     // This is the tested method -----------
                     std::pair<bool, ArcOnEllipsoid> data = GeolibUtils::CreateArcTangentToTwoLines(fake_line_behind_rf_leg, fake_line_outbound_from_rf_leg_extended, radius);
                     ...
                     EXPECT_NEAR(Units::DegreesAngle(where_rf_began.GetLatitude()).value(), Units::DegreesAngle(rf_leg.GetStartPoint().GetLatitude()).value(), latitude_tolerance_degrees);
                     EXPECT_NEAR(Units::DegreesAngle(where_rf_began.GetLongitude()).value(), Units::DegreesAngle(rf_leg.GetStartPoint().GetLongitude()).value(), longitude_tolerance_degrees);
                     std::pair<Units::Length, Units::SignedAngle> return_info = where_rf_began.CalculateRelationshipBetweenPoints(rf_leg.GetStartPoint());
                     ...
                     EXPECT_NEAR(Units::DegreesAngle(where_rf_ends.GetLatitude()).value(), Units::DegreesAngle(rf_leg.GetEndPoint().GetLatitude()).value(), latitude_tolerance_degrees);
                     EXPECT_NEAR(Units::DegreesAngle(where_rf_ends.GetLongitude()).value(), Units::DegreesAngle(rf_leg.GetEndPoint().GetLongitude()).value(), longitude_tolerance_degrees);
                     ...
                     EXPECT_NEAR(Units::NauticalMilesLength(radius).value(), Units::NauticalMilesLength(rf_leg.GetRadius()).value(), 1e-10);
                     // -------------------------------------
            ...
            for (auto item : waypoints_to_connect)
            {
               ...
               if (wpt.GetName() != "HIMOM")
               {
                  ...
                  else
                  {
                     const LatitudeLongitudePoint where_rf_began = LatitudeLongitudePoint::CreateFromWaypoint(prev_waypoint);
                     const LatitudeLongitudePoint where_rf_ends = LatitudeLongitudePoint::CreateFromWaypoint(wpt);
                     const LatitudeLongitudePoint projected_start_point = where_rf_began.ProjectDistanceAlongCourse(Units::NauticalMilesLength(100), course_enu_tangency_end_of_prev_shape + Units::SignedRadiansAngle(180));
                     const LineOnEllipsoid fake_inbound_line_to_rf_leg = LineOnEllipsoid::CreateFromPoints(projected_start_point, where_rf_began);

                     // This is the tested method -----------
                     ArcOnEllipsoid rf_leg = GeolibUtils::CreateArcFromInboundShapeAndEndPoint(&fake_inbound_line_to_rf_leg, where_rf_ends);
                     // -------------------------------------
                     ...
                     // Expect that the arc's radius will be mathematically close to the input radius
                     EXPECT_NEAR(Units::NauticalMilesLength(radius).value(), Units::NauticalMilesLength(rf_leg.GetRadius()).value(), distance_tolerance_meters);
                     // -------------------------------------
         ...
         TEST(GeolibUtils, CreateArcInboundLineAndEndPoint_valid_case2)
         {
            ...
            std::vector<std::tuple<bool, Waypoint, Units::Length>> waypoints_to_connect = {
                // tuple holds: bool for is_line, waypoint, rf_radius
                std::make_tuple(true, Waypoint("FERNN", Units::DegreesAngle(33.97281666666667), Units::DegreesAngle(-116.54705277777778), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::infinity()),
                std::make_tuple(true, Waypoint("JEVOK", Units::DegreesAngle(33.88010833333333), Units::DegreesAngle(-116.46062500000001), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::infinity()),
                std::make_tuple(false, Waypoint("CUSIT", Units::DegreesAngle(33.95426944444445), Units::DegreesAngle(-116.29662777777777), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::NauticalMilesLength(4.72)),
                std::make_tuple(true, Waypoint("HOPLI", Units::DegreesAngle(34.04785277777778), Units::DegreesAngle(-116.33561388888889), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::infinity()),
                std::make_tuple(false, Waypoint("YOCUL", Units::DegreesAngle(34.111425000000004), Units::DegreesAngle(-116.40140833333334), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::NauticalMilesLength(6.83)),
                std::make_tuple(false, Waypoint("WASAK", Units::DegreesAngle(34.09063888888889), Units::DegreesAngle(-116.52534166666666), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED), Units::NauticalMilesLength(5.00)),
            };
            Waypoint prev_waypoint;
            Units::SignedAngle course_enu_tangency_end_of_prev_shape;
            std::vector<const ShapeOnEllipsoid *> simple_path_along_ground;
            ...
            for (auto item : waypoints_to_connect)
            {
               ...
               if (wpt.GetName() != "FERNN")
               {
                  ...
                  else
                  {
                     ...
                     // This is the tested method -----------
                     std::vector<std::pair<bool, LatitudeLongitudePoint>> intersection_data = GeolibUtils::CalculateLineArcIntersectionPoints(fake_inbound_line_to_rf_leg, rf_leg);
                     // -------------------------------------
         ...
         TEST(LineOnEllipsoid, GetRelativeDirection)
         {
            ...
            const std::vector<Waypoint> start_points = {
                Waypoint("kphx_airport", Units::DegreesAngle(33.4342778), Units::DegreesAngle(-112.0115833), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED),
                Waypoint("ksfo_airport", Units::DegreesAngle(37.6188056), Units::DegreesAngle(-122.3754167), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED),
                Waypoint("kden_airport", Units::DegreesAngle(39.8616667), Units::DegreesAngle(-104.6731667), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED),
                Waypoint("ksfo_airport", Units::DegreesAngle(42.3629444), Units::DegreesAngle(-71.0063889), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED),
                Waypoint("katl_airport", Units::DegreesAngle(33.6366996), Units::DegreesAngle(-84.4278640), Units::ZERO_LENGTH, Units::ZERO_LENGTH, Units::ZERO_SPEED, Units::ZERO_LENGTH, Units::ZERO_SPEED)};

            const Units::MetersLength distance_off_line(1e-5); // this should be small to test navigation situations that are "very close" to the line being followed
            for (const Waypoint wpt : start_points)
            {
               ...
               {
                  const Units::SignedRadiansAngle
                      course_enu = GeolibUtils::ConvertCourseFromNedToEnu(Units::UnsignedRadiansAngle(crs_ned));
                  const LatitudeLongitudePoint
                      end_point = start_point.ProjectDistanceAlongCourse(Units::NauticalMilesLength(100), course_enu);
                  ...
                  // This is the tested method -----------
                  ShapeOnEllipsoid::DIRECTION_RELATIVE_TO_SHAPE
                      actual_dir1 = line_on_ellipsoid.GetRelativeDirection(test_point_actually_on_line);
                  EXPECT_TRUE(actual_dir1 == ShapeOnEllipsoid::ON_SHAPE);
                  // -------------------------------------

                  // Test: point to left result
                  const LatitudeLongitudePoint
                      test_point_actually_left_of_line = test_point_actually_on_line.ProjectDistanceAlongCourse(
                          distance_off_line,
                          GeolibUtils::ConvertCourseFromNedToEnu(Units::UnsignedRadiansAngle(crs_ned) - Units::RadiansAngle(M_PI_2)));
                  // This is the tested method -----------
                  ShapeOnEllipsoid::DIRECTION_RELATIVE_TO_SHAPE
                      actual_dir2 = line_on_ellipsoid.GetRelativeDirection(test_point_actually_left_of_line);
                  EXPECT_TRUE(actual_dir2 == ShapeOnEllipsoid::LEFT_OF_SHAPE);
                  // -------------------------------------

                  // Test: point to right result
                  const LatitudeLongitudePoint
                      test_point_actually_right_of_line = test_point_actually_on_line.ProjectDistanceAlongCourse(
                          distance_off_line,
                          GeolibUtils::ConvertCourseFromNedToEnu(Units::UnsignedRadiansAngle(crs_ned) + Units::RadiansAngle(M_PI_2)));
                  // This is the tested method -----------
                  ShapeOnEllipsoid::DIRECTION_RELATIVE_TO_SHAPE
                      actual_dir3 = line_on_ellipsoid.GetRelativeDirection(test_point_actually_right_of_line);
         ...
         TEST(EarthModel, ToUnitVector)
         {
            ...
            EXPECT_NEAR(unity.value(), Units::MetersLength(Units::sqrt(Units::sqr(unit_vector.x) + Units::sqr(unit_vector.y) + Units::sqr(unit_vector.z))).value(), 1e-10);
         }
         ...
         TEST(ArcOnEllipsoid, GetRelativeDirection)
         {
            ...
            const Units::MetersLength distance_off_arc(1e-5); // this should be small to test navigation situations that are "very close" to the shape being followed
            const Units::NauticalMilesLength arc_radius_approximate(5);
            const geolib_idealab::ArcDirection arc_directions[2] = {geolib_idealab::ArcDirection::CLOCKWISE, geolib_idealab::ArcDirection::COUNTERCLOCKWISE};
            ...
            for (const Waypoint wpt : center_points)
            {
               ...
               for (geolib_idealab::ArcDirection arc_direction : arc_directions)
               {
                  ...
                  for (double arc_start_course_ned = 3 * M_PI / 2; arc_start_course_ned <= 2 * M_PI; arc_start_course_ned += M_PI / 2)
                  {
                     const Units::SignedRadiansAngle arc_start_course_enu = GeolibUtils::ConvertCourseFromNedToEnu(Units::UnsignedRadiansAngle(arc_start_course_ned));
                     LatitudeLongitudePoint line_end_point = center_point_approximate.ProjectDistanceAlongCourse(arc_radius_approximate, arc_start_course_enu);
                     LatitudeLongitudePoint line_start_point, arc_end_point;
                     if (arc_direction == geolib_idealab::ArcDirection::CLOCKWISE)
                     {
                        line_start_point = line_end_point.ProjectDistanceAlongCourse(arc_radius_approximate, arc_start_course_enu + Units::PI_RADIANS_ANGLE / 2);
                        arc_end_point = center_point_approximate.ProjectDistanceAlongCourse(arc_radius_approximate, arc_start_course_enu - Units::PI_RADIANS_ANGLE / 2);
                     }
                     else
                     {
                        line_start_point = line_end_point.ProjectDistanceAlongCourse(arc_radius_approximate, arc_start_course_enu - Units::PI_RADIANS_ANGLE / 2);
                     ...
                     const ArcOnEllipsoid arc_on_ellipsoid = GeolibUtils::CreateArcFromInboundShapeAndEndPoint(&inbound_line, arc_end_point);
                     ...
                     if (arc_direction == geolib_idealab::ArcDirection::CLOCKWISE)
                     {
                        test_point_actually_on_arc = arc_on_ellipsoid.GetCenterPoint().ProjectDistanceAlongCourse(arc_on_ellipsoid.GetRadius(), arc_start_course_enu - Units::PI_RADIANS_ANGLE / 4);
                     }
                     else
                     {
                        test_point_actually_on_arc = arc_on_ellipsoid.GetCenterPoint().ProjectDistanceAlongCourse(arc_on_ellipsoid.GetRadius(), arc_start_course_enu + Units::PI_RADIANS_ANGLE / 4);
                     ...
                     // This is the tested method -----------
                     ShapeOnEllipsoid::DIRECTION_RELATIVE_TO_SHAPE actual_dir1 = arc_on_ellipsoid.GetRelativeDirection(test_point_actually_on_arc);
                     EXPECT_TRUE(actual_dir1 == ShapeOnEllipsoid::ON_SHAPE);
                     ...
                     if (arc_direction == geolib_idealab::ArcDirection::CLOCKWISE)
                     {
                        test_point_actually_left_of_line = arc_on_ellipsoid.GetCenterPoint().ProjectDistanceAlongCourse(
                            ...
                            GeolibUtils::ConvertCourseFromNedToEnu(Units::UnsignedRadiansAngle(arc_start_course_ned + M_PI_4)));
                        // This is the tested method -----------
         ...
         TEST(LineOnEllipsoid, GetDistanceToEndPoint)
         {
            ...
            const LatitudeLongitudePoint test_point_on_line = zero_zero.ProjectDistanceAlongCourse(test_line_north.GetShapeLength() / 2, test_line_north.GetForwardCourseEnuAtStartPoint());
            ...
            const LatitudeLongitudePoint test_point_not_on_line = test_point_on_line.ProjectDistanceAlongCourse(Units::NauticalMilesLength(1), test_line_north.GetForwardCourseEnuAtStartPoint() + Units::PI_RADIANS_ANGLE / 2);

            // This is the test method ------------------
            const Units::MetersLength actual_distance_to_end_point_2 = test_line_north.GetDistanceToEndPoint(test_point_not_on_line);
            // ------------------------------------------
         ...
         TEST(ArcOnEllipsoid, GetDistanceToEndPoint)
         {
            ...
            const LatitudeLongitudePoint arc_end_point = one_zero.ProjectDistanceAlongCourse(Units::NauticalMilesLength(5), inbound_line_to_arc.GetForwardCourseEnuAtEndPoint() - Units::PI_RADIANS_ANGLE / 2);
            const ArcOnEllipsoid arc_on_ellipsoid = GeolibUtils::CreateArcFromInboundShapeAndEndPoint(&inbound_line_to_arc, arc_end_point);
            LatitudeLongitudePoint test_point_on_arc;
            if (arc_on_ellipsoid.GetArcDirection() == geolib_idealab::ArcDirection::CLOCKWISE)
            {
               test_point_on_arc = arc_on_ellipsoid.GetCenterPoint().ProjectDistanceAlongCourse(arc_on_ellipsoid.GetRadius(), arc_on_ellipsoid.GetStartAzimuthEnu() + arc_on_ellipsoid.GetArcAngularExtent() / 2);
            }
            else
            {
               test_point_on_arc = arc_on_ellipsoid.GetCenterPoint().ProjectDistanceAlongCourse(arc_on_ellipsoid.GetRadius(), arc_on_ellipsoid.GetStartAzimuthEnu() - arc_on_ellipsoid.GetArcAngularExtent() / 2);
            ...
            if (arc_on_ellipsoid.GetArcDirection() == geolib_idealab::ArcDirection::CLOCKWISE)
            {
               test_point_not_on_line = arc_on_ellipsoid.GetCenterPoint().ProjectDistanceAlongCourse(arc_on_ellipsoid.GetRadius() + Units::NauticalMilesLength(1), arc_on_ellipsoid.GetStartAzimuthEnu() + arc_on_ellipsoid.GetArcAngularExtent() / 2);
            }
            else
            {
               test_point_not_on_line = arc_on_ellipsoid.GetCenterPoint().ProjectDistanceAlongCourse(arc_on_ellipsoid.GetRadius() - Units::NauticalMilesLength(1), arc_on_ellipsoid.GetStartAzimuthEnu() - arc_on_ellipsoid.GetArcAngularExtent() / 2);
            ...
            // This is the test method ------------------
            const Units::MetersLength actual_distance_to_end_point_2 = arc_on_ellipsoid.GetDistanceToEndPoint(test_point_not_on_line);
            // ------------------------------------------
